...
if($confirm == '42')
{
   ...
   echo'<table width="800" border="'.ooborder().'">
   ...
   {
      $arr = mysql_fetch_array(mysql_query("SELECT sum(".$row['display'].") AS ".$row['display']." FROM `stk`"));
      $arr2 = mysql_fetch_array(mysql_query("SELECT sum(".$row['display'].") AS ".$row['display']." FROM `used`"));
      $arra = mysql_fetch_array(mysql_query("SELECT sum(".$row['display'].") AS ".$row['display']." FROM `stk`"));
      $arrb = mysql_fetch_array(mysql_query("SELECT sum(".$row['display'].") AS ".$row['display']." FROM `used`"));
      $result3 = $arra[$row['display']] - $arrb[$row['display']];
      echo '<tr>';
      echo '<td width="100" bgcolor="'.setcolor('tb_color2').'" >'.$row['display'].'</td><td width="100" bgcolor="'.setcolor('tb_color3').'" align="center">'.$arr[$row['display']].'</td><td width="100" bgcolor="'.setcolor('tb_color3').'" align="center">'.$arrb[$row['display']].'</td><td width="100" bgcolor="'.setcolor('tb_color3').'" align="center">';
   ...
   echo '<table width="800" border="'.ooborder().'"><tr bgcolor="'.setcolor('tb_color1').'"><th>Item</th><th>sold pcs</th><th>sold</th></tr>';
   ...
   {
      $ausg = mysql_fetch_array(mysql_query("SELECT sum(`preis`) AS total FROM `expn` WHERE `name` = '".$aname['name']."'"));
